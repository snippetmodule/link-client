...
const initialState = {
    server: [],
    push: [],
    enabled: ReactNative.Platform.OS === 'ios' ? null : true,
    registered: false,
    seen: {},
};
...
export function notifications(state: State = initialState, action: Action): State {
    switch (action.type) {
...
function fromParseObject(object: any): Notification {
    return {
        id: object.id,
